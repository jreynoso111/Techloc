...
(function () {
  ...
  const updateHeaderAccount = async (session, profileEmail) => {
    ...
    if (!session?.user) {
      ...
      window.dispatchEvent(new CustomEvent('auth:account-updated', { detail: { email: 'Account' } }));
      accountName.textContent = 'Account';
      return;
    }

    const label = session.user.email || profileEmail || 'Account';
    window.currentUserEmail = label;
    window.dispatchEvent(new CustomEvent('auth:account-updated', { detail: { email: label } }));
